...
//        AppAsset::register($this->getView());
        ...
        if ($_SERVER['REQUEST_URI'] != '/checkout' && strlen($this->session->get('email', ' ')) < 3) {
            ...
            Alerts::setMessage("Oops, you're session has timed out");
    ...
    public function action4()
    {
        ...
        $model = new Billing();
        foreach ($model->attributes as $key => $item) {
            $model->$key = $this->session->get('billing.' . $key);
        }
    ...
    public function action5()
    {
        ...
        if (isset($_POST['Checkout'])) {
            ...
            $order->shipping   = 0.00;
            $order->tax        = 0.00;
            $order->total      = number_format($order->subtotal + $order->shipping + $order->tax + 0.00, 2);

            /* Check transaction is okay */
            $trans_id   = '';
            $trans_fee  = 0.00;
            $trans_desc = '';
            ...
            $checkout_transaction->amount              = $order->total;
            $checkout_transaction->transaction_type_id = $this->session->get('transaction_type', 0);
            $checkout_transaction->transaction_id      = $trans_id;
...
//            print_r($checkout_shipping); exit;
            ...
            foreach ($cart->getCartItems() as $item) {
                ...
                $checkout_link->shipping_id = $checkout_shipping->id;
